...
def test_forward_abspos(setup_module):
    torch.manual_seed(42)
    module = setup_module
...
def test_forward_no_batch_idx(setup_module):
    module = setup_module
    input_pos = torch.rand(10, 3)
    supernode_idxs = torch.tensor([0, 1, 5, 6])
    batch_idx = None
...
def test_compute_src_and_dst_indices(setup_module):
