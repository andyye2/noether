...
from research.multi_fidelity.tools.materialize_nested_manifests import build_manifest, canonical_bytes
...
def test_statistics_cli_binding_accepts_frozen_protocol() -> None:
    """The frozen repository protocol and rematerialized manifest are bound."""
    ...
    validate_frozen_protocol_for_statistics(protocol, manifest)


def test_statistics_cli_binding_rejects_draft_protocol(tmp_path: Path) -> None:
    """Target-label statistics cannot start from a draft protocol."""
    repo_root = Path(__file__).parents[4]
    frozen_protocol = repo_root / "research/multi_fidelity/experiment_protocol.yaml"
    manifest = repo_root / "research/multi_fidelity/evidence/manifests/drivaerml_nested_seed1103.json"
    protocol = yaml.safe_load(frozen_protocol.read_text(encoding="utf-8"))
    ...
    with pytest.raises(ValueError, match="statistics execution requires protocol status"):
        validate_frozen_protocol_for_statistics(draft_protocol, manifest)
